...
// Set up the connection string from environment
process.env.POSTGRES_URL = process.env.POSTGRES_URL || process.env.POSTGRES_URLPGSQL;
...
async function initDB() {
  ...
  if (rows.length === 0) {
    ...
    await sql`
      ...
      VALUES ('admin-001', 'admin', ${adminHash}, 'admin', 'Admin')
      ON CONFLICT DO NOTHING
    `;
    await sql`
      INSERT INTO users (id, username, password, role, name)
      VALUES ('checker-001', 'Bill checker', ${checkerHash}, 'checker', 'Bill Checker')
      ON CONFLICT DO NOTHING
    `;
    // Seed demo farmer
    ...
    await sql`
      ...
      VALUES ('farmer-001', '0032', ${farmerHash}, 'farmer', '0032', 'Shiv Bharwad', '9876543210', 'Bharwad Vas')
      ON CONFLICT DO NOTHING
    `;
  }
}
