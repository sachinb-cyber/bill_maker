const { Pool } = require('pg');

// Disable strict SSL validation for Supabase self-signed certificates
process.env.NODE_TLS_REJECT_UNAUTHORIZED = '0';

// Create a connection pool using the environment variable
const connectionString = process.env.POSTGRES_URLPGSQL || process.env.POSTGRES_URL;
console.log('DB Config:', {
  host: connectionString?.split('@')?.[1]?.split(':')?.[0],
  port: connectionString?.split(':')?.[4],
  hasPassword: !!connectionString?.split(':')?.[2]
});

const pool = new Pool({
  connectionString,
  ssl: {
    rejectUnauthorized: false,
    sslmode: 'require'
  },
  max: 5,
  idleTimeoutMillis: 30000,
  connectionTimeoutMillis: 10000
});

// Handle pool errors
pool.on('error', (err) => {
  console.error('Pool error:', err);
});

// Helper function to run SQL queries with template literals
const sql = async (query, values = []) => {
  const client = await pool.connect();
  try {
    const result = await client.query(query, values);
    // Deep clone rows to ensure they're plain objects
    const plainRows = result.rows.map(row => {
      const plainRow = {};
      for (const key in row) {
        plainRow[key] = row[key];
      }
      return plainRow;
    });
    return {
      rows: plainRows,
      rowCount: result.rowCount,
      command: result.command
    };
  } catch (err) {
    console.error('Query error:', err.message, err.stack);
    throw err;
  } finally {
    client.release();
  }
};

// Initialize all tables
async function initDB() {
  const client = await pool.connect();
  try {
    // First, try to update existing foreign key constraints to use CASCADE
    try {
      await client.query(`
        ALTER TABLE bills 
        DROP CONSTRAINT IF EXISTS bills_farmer_id_fkey;
      `);
      await client.query(`
        ALTER TABLE bills 
        ADD CONSTRAINT bills_farmer_id_fkey 
        FOREIGN KEY (farmer_id) REFERENCES users(id) ON DELETE CASCADE;
      `);
    } catch (e) {
      // Constraint might not exist or already be correct
    }
    
    try {
      await client.query(`
        ALTER TABLE bill_requests 
        DROP CONSTRAINT IF EXISTS bill_requests_farmer_id_fkey;
      `);
      await client.query(`
        ALTER TABLE bill_requests 
        ADD CONSTRAINT bill_requests_farmer_id_fkey 
        FOREIGN KEY (farmer_id) REFERENCES users(id) ON DELETE CASCADE;
      `);
    } catch (e) {
      // Constraint might not exist or already be correct
    }
    
    try {
      await client.query(`
        ALTER TABLE payments 
        DROP CONSTRAINT IF EXISTS payments_farmer_id_fkey;
      `);
      await client.query(`
        ALTER TABLE payments 
        ADD CONSTRAINT payments_farmer_id_fkey 
        FOREIGN KEY (farmer_id) REFERENCES users(id) ON DELETE CASCADE;
      `);
    } catch (e) {
      // Constraint might not exist or already be correct
    }
    
    await client.query(`
      CREATE TABLE IF NOT EXISTS users (
        id TEXT PRIMARY KEY,
        username TEXT UNIQUE NOT NULL,
        password TEXT NOT NULL,
        role TEXT NOT NULL CHECK (role IN ('admin','farmer','checker')),
        farmer_code TEXT,
        name TEXT,
      dairy_name TEXT DEFAULT 'SHRI KRISHNA DUDH DAIRY',
      mobile TEXT,
      village TEXT,
      milk_type TEXT DEFAULT 'Cow',
      l_min NUMERIC DEFAULT 37.5,
      l_max NUMERIC DEFAULT 43.5,
      f_min NUMERIC DEFAULT 4.1,
      f_max NUMERIC DEFAULT 4.9,
      target NUMERIC DEFAULT 103000,
      created_at TIMESTAMPTZ DEFAULT NOW()
    )
  `);

    await client.query(`
      CREATE TABLE IF NOT EXISTS bills (
        id TEXT PRIMARY KEY,
        farmer_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
        farmer_code TEXT NOT NULL,
        farmer_name TEXT NOT NULL,
        dairy_name TEXT NOT NULL,
        milk_type TEXT NOT NULL,
        month_index INTEGER NOT NULL,
        month_name TEXT NOT NULL,
        year TEXT NOT NULL,
        days INTEGER NOT NULL,
        total_entries INTEGER NOT NULL,
        total_litre NUMERIC NOT NULL,
        avg_fat NUMERIC NOT NULL,
        total_amount NUMERIC NOT NULL,
        records JSONB NOT NULL,
        cfg JSONB,
        bill_id TEXT UNIQUE NOT NULL,
        status TEXT DEFAULT 'active',
        created_at TIMESTAMPTZ DEFAULT NOW(),
        updated_at TIMESTAMPTZ DEFAULT NOW()
      )
    `);

    await client.query(`
      CREATE TABLE IF NOT EXISTS bill_requests (
        id TEXT PRIMARY KEY,
        farmer_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
        farmer_code TEXT NOT NULL,
        farmer_name TEXT NOT NULL,
        month_name TEXT NOT NULL,
        month_index INTEGER NOT NULL,
        year TEXT NOT NULL,
        amount_paid NUMERIC NOT NULL,
        payment_mode TEXT NOT NULL,
        reference TEXT,
        note TEXT,
        status TEXT DEFAULT 'pending' CHECK (status IN ('pending','approved','rejected')),
        admin_note TEXT,
        bill_id TEXT,
        created_at TIMESTAMPTZ DEFAULT NOW(),
        resolved_at TIMESTAMPTZ
      )
    `);

    await client.query(`
      CREATE TABLE IF NOT EXISTS payments (
        id TEXT PRIMARY KEY,
        farmer_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
        farmer_code TEXT NOT NULL,
        farmer_name TEXT NOT NULL,
        month_name TEXT NOT NULL,
        year TEXT NOT NULL,
        amount NUMERIC NOT NULL,
        payment_mode TEXT NOT NULL,
        reference TEXT,
        status TEXT DEFAULT 'paid' CHECK (status IN ('paid','partial','pending')),
        created_at TIMESTAMPTZ DEFAULT NOW()
      )
    `);

    // Seed admin and checker users if not exist
    const bcrypt = require('bcryptjs');
    const adminCheckResult = await client.query(`SELECT id FROM users WHERE role='admin' LIMIT 1`);
    const rows = adminCheckResult.rows || [];
    
    console.log('Admin check result:', { rowCount: adminCheckResult.rowCount, rows: rows.length });
    
    if (rows.length === 0) {
      console.log('Creating admin user...');
      const adminHash = await bcrypt.hash('Sachin@12389qweasd#4', 10);
      const checkerHash = await bcrypt.hash('Sachin@123098@#1asdd', 10);
      
      const adminInsert = await client.query(
        `INSERT INTO users (id, username, password, role, name)
         VALUES ('admin-001', 'admin', $1, 'admin', 'Admin')
         ON CONFLICT DO NOTHING`,
        [adminHash]
      );
      console.log('Admin insert result:', { rowCount: adminInsert.rowCount });
      
      await client.query(
        `INSERT INTO users (id, username, password, role, name)
         VALUES ('checker-001', 'Bill checker', $1, 'checker', 'Bill Checker')
         ON CONFLICT DO NOTHING`,
        [checkerHash]
      );
      
      // Seed demo farmer
      const farmerHash = await bcrypt.hash('9876543210', 10);
      await client.query(
        `INSERT INTO users (id, username, password, role, farmer_code, name, mobile, village)
         VALUES ('farmer-001', '0032', $1, 'farmer', '0032', 'Shiv Bharwad', '9876543210', 'Bharwad Vas')
         ON CONFLICT DO NOTHING`,
        [farmerHash]
      );
      console.log('Database seeding completed');
    } else {
      console.log('Admin user already exists');
    }
  } finally {
    client.release();
  }
}

module.exports = { pool, sql, initDB };
