...
module.exports = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      'SELECT b.*, u.name as farmer_name_db FROM bills b JOIN users u ON b.farmer_id = u.id WHERE b.farmer_code=$1 AND b.month_index=$2 AND b.year=$3 LIMIT 1',
      [farmer_code, month_index, year]
    );

    if (!result.rows.length) return res.json({ status: 'NOT_FOUND', genuine: false });
